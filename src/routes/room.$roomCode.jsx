...
function RoomPage() {
	...
	const { status, peerCount, peerConnectionState, remoteStream, isSharing, error, startSharing, stopSharing } = useWatchParty(roomCode.toUpperCase());
	useEffect(() => {
		if (!copied) return;
	...
	const statusLabel = status === "full" ? "Room is full" : status === "connecting" ? "Connecting…" : status === "waiting" ? "Waiting for your friend" : peerConnectionState === "connected" ? "Peer-to-peer connected" : "Both here — ready to share";
	if (status === "full") {
		return <main className="mx-auto flex min-h-screen max-w-md flex-col items-center justify-center gap-4 px-6 text-center">
	...
	return <main className="mx-auto w-full max-w-5xl px-5 py-8">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <div className="mt-1 flex items-center gap-2">
            ...
            <button type="button" className={btnGhost} aria-label="Copy room code" onClick={copyCode}>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1.5 text-sm">
            <Wifi className={`size-4 ${peerConnectionState === "connected" ? "text-primary" : "text-muted-foreground"}`} />
            {statusLabel}
          </span>
      ...
      <VideoPlayer stream={remoteStream} placeholder={peerCount < 2 ? "Share the room code above. The screen appears here once your friend joins and shares." : "Connected. Either of you can hit “Share tab” to start."} />

      <div className="mt-5 flex flex-wrap items-center gap-3">
        <ShareButton isSharing={isSharing} disabled={peerCount < 2} onStart={() => void startSharing()} onStop={stopSharing} />
        {isSharing && <p className="text-sm text-muted-foreground">
            You are sharing. Pick “Chrome Tab” + “Share tab audio” for sound.
          </p>}
        {peerCount < 2 && !isSharing && <p className="text-sm text-muted-foreground">Sharing unlocks when the second person joins.</p>}
      </div>
