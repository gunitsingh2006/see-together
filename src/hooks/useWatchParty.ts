...
export type RoomStatus = "connecting" | "waiting" | "ready" | "full" | "error";
...
export function useWatchParty(roomCode: string) {
  ...
  const ensurePeerConnection = useCallback(() => {
    ...
    pc.onnegotiationneeded = async () => {
      try {
        ...
        if (pc.localDescription) send({ type: "desc", description: pc.localDescription.toJSON() });
      } catch (err) {
  ...
  const stopSharing = useCallback(() => {
    ...
    if (pc) {
      pc.getSenders().forEach((sender) => {
        if (sender.track) pc.removeTrack(sender); // triggers renegotiation
      });
    }
  ...
  const handleSignal = useCallback(
    ...
    [clientId, ensurePeerConnection, send],
  );
  ...
  }, [roomCode, clientId, ensurePeerConnection, handleSignal, teardownPeer]);
